...
class ClientWatcher extends Watcher {
    ...
    perHourCalc(baserate,seconds,rates){
        const hours = Math.floor(seconds / 3600);
        const minutes = Math.round((seconds % 3600) / 60);
        return { total:((hours * baserate) + (minutes/60 * baserate)).toFixed(2),expected:(((hours * baserate) + (minutes/60 * baserate))*rates).toFixed(2),received:(((hours * baserate) + (minutes/60 * baserate))*rates).toFixed(2)};
    }
    ...
    computeSalary(selectedMonth,quarter,baseRate,received){
        this.callFunc(CLIENT.ComputeSalary,{selectedMonth,quarter,baseRate,received}).then((data) => {
            if(data){
                this.#payrollData = data
                Toast({text:'Calculate Succefully'})
            }
            this.activateWatcher()
        }).catch((err) => {
            console.log(err);
